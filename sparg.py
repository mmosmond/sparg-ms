...
def _sample_times(PATH_TO_RELATE, infile, coalfile, u = 1.25e-8, M = 10, first_bp = 1, last_bp = 1, outfile = None):
    'sample branch lengths at bp M times'
...
def _get_epochs(coalfile):
    'get epcoh times and population size in each from coal file'
...
def get_atimes(stimes, time, focal_index):
    """
    get_atimes(sts, time, focal_index)
    ...
    >>> get_atimes(stimes, 7.5, 0)
    array([2.5, 0., 2.5])
    
    >>> stimes = np.array([[10,0],[0,10]])
    >>> get_atimes(stimes, 12.5, 0)
    array([-2.5, -2.5, -2.5])
    """
...
def get_alltimes(stimes, atimes, keep=None):
...
def get_ahatavar(alltimes, locations, tsplits=[], SIGMA=None, ahat_only=False):
    """
    get_ahatavar()
    
    Calculate mean and covariance in ancestor location
    """
...
def _ancestor_location_meanloglikelihood(coal_times, pcoals, shared_times, samples, locations, focal_node, keep=None, phi=1, SIGMA=None, locus=0, time=0, tCutoff=1e6, importance=True, tsplits=[], BLUP=False):
    ...
    for tree,sample in enumerate(samples[locus]): 
        ...
        atimes = get_atimes(sts, time, m) #get shared times between sample lineages and ancestor lineage
        if keep is None:
            ...
            alltimes = get_alltimes(sts, atimes, keep=ixs) #combine into one covariance matrix (and filter to keep)
        ...
        if BLUP:
            ...
            ahat = get_ahatavar(alltimes, locs, tsplits, SIGMA, ahat_only=BLUP) #the mle only
            fs.append(ahat)

        else:

            try:
                ahat, avar = get_ahatavar(alltimes, locs, tsplits, SIGMA, ahat_only=BLUP) #mle and covariance
